...
#define get_clocks(clocks)						\
    do {    \
        uint32_t clocks_hi, clocks_lo;  \
        asm volatile ( \
            "rdtsc" : "=a" (clocks_lo), "=d" (clocks_hi)    \
        );  \
        clocks = (((uint64_t)clocks_hi) << 32) | ((uint64_t)clocks_lo); \
    } while (0)

/**
* asm volatile - execute as written
* rdtsc - lower 32 bit in EAX, upper 32 bit into EDX
* =a(clocks_lo) - store 32 bit from EAX into clocks_lo
* =d(clocks_hi) - store 32 bit from EDX into clocks_hi
* after rdtsc, combine high and low parts of counter
* while(0) - single statement 
* measures number of CPU cycles elapsed during operations
*/
