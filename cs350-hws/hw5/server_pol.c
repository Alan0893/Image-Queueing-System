...
{
    int retval = 0;
    sem_wait(queue_mutex);  // Protect the queue

    if (the_queue->available == 0) {
        retval = 1; // Queue full, return 1 to reject the request
    } else {
        if (the_queue->policy == QUEUE_FIFO) {
            // FIFO policy: Simply add the request at the write position
            ...
            // SJN policy: Add the request based on its length (timespec comparison)
        ...
        // Update queue availability and notify waiting threads
        the_queue->available--;
        sem_post(queue_notify);
    }

    sem_post(queue_mutex); // Release the queue lock
    return retval;
}
	...
	while((opt = getopt(argc, argv, "q:w:p:")) != -1) {
		...
		case 'p':
			if (strcmp(optarg, "FIFO") == 0) {
				...
				fprintf(stderr, "ERROR: Invalid policy specified. Use FIFO or SJN.\n");
